export class Game {
    ...
    display(): void {
      const mapPlayerToCharacter: { [name: number]: string } = {};
    ...
    getLowestEmptyLine(column: Array<number>): number | null | undefined { // ! ?number
    ...
    getDiagonalByHighestCellDescendingRight(lineIndex: number, columnIndex: number): number[] | null | undefined { // ! undefined necessaire ?
    ...
    getDiagonalByHighestCellDescendingLeft(lineIndex: number, columnIndex: number): number[] | null | undefined { // ! undefined necessaire ?
      const maxCroppedSquareSideLength = Math.min(
        this.height - lineIndex,
        columnIndex + 1
      );
    ...
    getAllDiagonals(): Array<Array<number>> {
      let diagonals : any[] = []; // ! Change any to correct value
