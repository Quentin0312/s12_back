...
for (let i = 0; i < LEARN_TIMES; i++) {
  ...
  const epsilon =
    EPSILON_INIT + ((EPSILON_FINAL - EPSILON_INIT) * i) / LEARN_TIMES;
  // console.log("epsilon", epsilon);

  // play a game of connect4
  const gameInfo = playGame(epsilon, myNetwork, display);
  // console.log("winner plays", gameInfo.winnerPlays);
  // console.log("loser plays", gameInfo.loserPlays);
  ...
  // adapt learning rate
  const learningRate =
    LR_INIT / (1 + ((LR_FINAL_FRACTION - 1) * i) / LEARN_TIMES);
  ...
  if (winnerBoardStates.length !== 0) {
    ...
    if (
      winnerPlays[winnerPlaysLength - 1] !== loserPlays[loserPlaysLength - 1]
    ) {
...
fs.writeFile(`networkWeights001.json`, json, "utf8", (err) => {
  if (err) throw err;
